...
Celula* new_celula(Personagens personagem){
...
void inserir(Personagens x, Pilha *p) {
    Celula *temp = new_celula(x);
    temp->prox = p->topo;
    p->topo = temp;
    p->size++;
}

Personagens remover(Pilha *p) {
   if (p->topo == NULL){
        printf("\nA pilha esta vazia!\n");
        exit(1);
    }
    Personagens tmp = p->topo->personagem;
    Celula *temp = p->topo;
    p->topo = p->topo->prox;
    p->size--;
    free(temp);
...
void mostrar (Pilha *p){
    ...
    for(Celula *i = p->topo; i!=NULL ;i = i->prox){
        printf("[%d ## %s ##", p->size - k , i->personagem.id);
...
void opcoes(char opcao[5], char Id[51], Personagens *listaPersonagens, Pilha *pilha){
    if(strcmp(opcao,"R") == 0){
        Personagens tmp = remover(pilha);
        printf("(R) %s\n", tmp.name);
    }else if(strcmp(opcao,"I") == 0){
        inserir(procuraPersonagem(listaPersonagens, Id), pilha);
    }
}
...
int main(){
    ...
    for(int i=0; i< n;i++){
        ...
        if(strcmp(ponteiro[0],"R") == 0){
            opcoes(ponteiro[0], " ",ptrPersonagens, &pilha);
        }else if(strcmp(ponteiro[0],"I") == 0){
            opcoes(ponteiro[0], ponteiro[1],ptrPersonagens, &pilha);
        }
    }
    mostrar(&pilha);
    free(ponteiroNovo);
