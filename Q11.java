...
class ListaDupla{
    ...
    public void Inserirfinal(Personagem x){
    ...
    public void Mostrar(){
        for(CelulaDupla j = primeiro.prox; j != null; j = j.prox){
            MyIO.print("[" +j.personagem.getId()+" ## "+j.personagem.getName()+" ## {");
    ...
    private void quicksort(CelulaDupla esq, CelulaDupla dir) {
        ...
        if (compareHouse(esq.personagem,j.personagem) < 0)  quicksort(esq, j);
        if (compareHouse(i.personagem,dir.personagem) > 0)  quicksort(i, dir);
    }

    private void swap(Personagem i, Personagem j) {
		Personagem temp = i;
		i = j;
		j = temp;
        //movimentacoes+=3;
...
public class Q11{
    ...
    public static void main(String[] args){
        ...
        for(int j=0; j<arrayPersonagens.length-1;j++){
            lista.Inserirfinal(arrayPersonagens[j]);
        }
        lista.quicksort();
        lista.Mostrar();
        Arq.openWrite("824137_quicksort2.txt");
        teclado.close();
