...
Personagens arrayPersonagens[MAXTAM];   
int tamanhoArray;               
void start(){
   tamanhoArray = 0;
}
void inserirInicio(Personagens x){
   if(tamanhoArray >= MAXTAM){
      printf("Erro ao inserir!");
      exit(1);
   } 
   for(int i = tamanhoArray; i > 0; i--){
      arrayPersonagens[i] = arrayPersonagens[i-1];
   }

   arrayPersonagens[0] = x;
   tamanhoArray++;
}
void inserirFim(Personagens x) {
   ...
   arrayPersonagens[tamanhoArray] = x;
   tamanhoArray++;
}
void inserir(Personagens x, int pos) {
   int i;
   if(tamanhoArray >= MAXTAM || pos < 0 || pos > tamanhoArray){
      printf("Erro ao inserir!");
      exit(1);
   }
   for(i = tamanhoArray; i > pos; i--){
      arrayPersonagens[i] = arrayPersonagens[i-1];
   }

   arrayPersonagens[pos] = x;
   tamanhoArray++;
}
...
Personagens removerFim() {
   ...
   return arrayPersonagens[--tamanhoArray];
}

Personagens remover(int pos) {
   int i;
   Personagens resp;

   if (tamanhoArray == 0 || pos < 0 || pos >= tamanhoArray) {
      printf("Erro ao remover!");
      exit(1);
   }

   resp = arrayPersonagens[pos];
   tamanhoArray--;
...
void opcoes(char opcao[5], char Id[51], char local[3],Personagens *listaPersonagens){
    if(strcmp(opcao,"RI") == 0){
        Personagens tmp = removerInicio();
        printf("(R) %s\n", tmp.name);
    }else if(strcmp(opcao,"RF") == 0){
        Personagens tmp = removerFim();
        printf("(R) %s\n", tmp.name);
    }else if(strcmp(opcao,"R*") == 0){
        Personagens tmp = remover(atoi(local));
        printf("(R) %s\n", tmp.name);
    }else if(strcmp(opcao,"II") == 0){
        inserirInicio(procuraPersonagem(listaPersonagens, Id));
    }else if(strcmp(opcao,"IF") == 0){
        inserirFim(procuraPersonagem(listaPersonagens, Id));
    }else if(strcmp(opcao,"I*") == 0){
        inserir(procuraPersonagem(listaPersonagens, Id), atoi(local));
    }
}
...
int main(){
    ...
    for(int i=0; i< n;i++){
        ...
        ponteiro[2] = strtok(NULL, " ");
        if(strcmp(ponteiro[0],"RI") == 0 || strcmp(ponteiro[0], "RF")==0){
            opcoes(ponteiro[0], " "," ",ptrPersonagens);
        }else if(strcmp(ponteiro[0],"R*") == 0){
            opcoes(ponteiro[0], " ", ponteiro[1],ptrPersonagens);
        }else if(strcmp(ponteiro[0],"I*") == 0){
            opcoes(ponteiro[0],ponteiro[2],ponteiro[1],ptrPersonagens);
        }else{
            opcoes(ponteiro[0],ponteiro[1], " ", ptrPersonagens);
        }
    }
